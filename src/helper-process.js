...
// 3D 渲染模式:存在 electron-helper 桥且未被显式禁用时使用(协议不变,渲染端换成 3D 虎鲸)
function useElectronBridge() {
...
function defaultCommand() {
  if (useElectronBridge()) {
    // 协议桥用纯 Node 运行(stdio 管道可靠);electron GUI 由桥内部拉起。
    // 使用 DSH 自身的 node(process.execPath),保证 PATH 无关。
...
function defaultArgs(command, helperPath) {
  if (useElectronBridge()) return [electronBridgePath]
